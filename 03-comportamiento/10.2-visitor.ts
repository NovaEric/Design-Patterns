...
class MaintenanceCostVisitor implements Visitor {
  visitCar(car: Car): void {
    const cost =
      car.getKilometers() * 0.1 + (2024 - car.getYear()) * 50;
  ...
  visitTruck(truck: Truck): void {
    ...
    const cost =
      truck.getKilometers() * 0.2 +
      (2024 - truck.getYear()) * 100 +
      truck.getLoadCapacity() * 10;
    // cost = Kilómetros recorridos * 0.2 + (2024 - Año de fabricación) * 100 + Capacidad de carga * 10 
...
class EmissionCheckVisitor implements Visitor {
  visitCar(car: Car): void {
    const passes = car.getYear() > 2010 && car.getKilometers() < 150_000;
    console.log(`Automóvil cumple con emisiones: ${passes ? 'Sí' : 'No'}`);
  }

  visitMotorcycle(motorcycle: Motorcycle): void {
    const passes = motorcycle.getYear() > 2012 && motorcycle.getKilometers() < 100_000;
    console.log(`Motocicleta cumple con emisiones: ${passes ? 'Sí' : 'No'}`);
  }

  visitTruck(truck: Truck): void {
    const passes = truck.getYear() > 2015 && truck.getKilometers() < 200_000;
